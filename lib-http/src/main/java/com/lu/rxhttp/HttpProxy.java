...
public class HttpProxy {
    ...
    private Object dispatcher(final Method method, Object[] args) {
        //请求类型
        if (!method.isAnnotationPresent(UrlEncoded.class)) {
            throw new RuntimeException("you need to add annotation ( @Form || @MultiPart || @Json ) to declare the quest Type");
        }

        UrlEncoded urlEncoded = method.getAnnotation(UrlEncoded.class);
        UrlEncoded.EncodeType type = urlEncoded.value();
        if (type == UrlEncoded.EncodeType.JSON) {
            return new JsonWork(baseUrl, responseBodyConvert).invoke(method, args);
        } else if (type == UrlEncoded.EncodeType.MULTIPART) {
            return new MultiPartWork(baseUrl, responseBodyConvert).invoke(method, args);
        } else {
            return new FormWork(baseUrl, responseBodyConvert).invoke(method, args);
        }
